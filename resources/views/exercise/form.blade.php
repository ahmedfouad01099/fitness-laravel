...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            <form action="{{ route('exercise.update', $id) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                <label for="bodypart_ids" class="form-control-label">{{ __('message.bodypart') }} <span class="text-danger">*</span></label>
                                <select name="bodypart_ids[]" id="bodypart_ids" class="select2js form-group bodypart" multiple data-placeholder="{{ __('message.select_name',[ 'select' => __('message.bodypart') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'bodypart']) }}" required>
                                    @if(isset($selected_bodypart) && count($selected_bodypart) > 0)
                                        @foreach($selected_bodypart as $key => $value)
                                            <option value="{{ $key }}" {{ in_array($key, old('bodypart_ids', [])) ? 'selected' : '' }}>{{ $value }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="equipment_id" class="form-control-label">{{ __('message.equipment') }}</label>
                                <a id="equipment_clear" class="float-end" href="javascript:void(0)">{{ __('message.l_clear') }}</a>
                                <select name="equipment_id" id="equipment_id" class="select2js form-group equipment" data-placeholder="{{ __('message.select_name',[ 'select' => __('message.equipment') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'equipment']) }}">
                                    @if(isset($id) && $data->equipment)
                                        <option value="{{ $data->equipment->id }}" {{ old('equipment_id') == $data->equipment->id ? 'selected' : '' }}>{{ $data->equipment->title }}</option>
                                    @endif
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="level_id" class="form-control-label">{{ __('message.level') }} <span class="text-danger">*</span></label>
                                <select name="level_id" id="level_id" class="select2js form-group level" data-placeholder="{{ __('message.select_name',[ 'select' => __('message.level') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'level']) }}" required>
                                    @if(isset($id) && $data->level)
                                        <option value="{{ $data->level->id }}" {{ old('level_id') == $data->level->id ? 'selected' : '' }}>{{ $data->level->title }}</option>
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="form-group col-md-4">
                                <label for="is_premium" class="form-control-label">{{ __('message.is_premium') }}</label>
                                <div class="">
                                    <input type="hidden" name="is_premium" value="0">
                                    <input type="checkbox" name="is_premium" id="is_premium" value="1" class="form-check-input" {{ old('is_premium', isset($data) ? $data->is_premium : 0) ? 'checked' : '' }}>
                                    <label class="custom-control-label" for="is_premium"></label>
                                </div>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="video_type" class="form-control-label">{{ __('message.video_type') }} <span class="text-danger">*</span></label>
                                <select name="video_type" id="video_type" class="form-control select2js video_type" required>
                                    <option value="">{{ __('message.select_status') }}</option>
                                    <option value="url" {{ old('video_type', isset($data) ? $data->video_type : '') == 'url' ? 'selected' : '' }}>{{ __('message.url') }}</option>
                                    <option value="upload_video" {{ old('video_type', isset($data) ? $data->video_type : '') == 'upload_video' ? 'selected' : '' }}>{{ __('message.upload_video') }}</option>
                                </select>
                            </div>
                            <div class="form-group col-md-4 video_url">
                                <label for="video_url" class="form-control-label">{{ __('message.video_url') }}</label>
                                <input type="url" name="video_url" id="video_url" value="{{ old('video_url', isset($data) ? $data->video_url : '') }}" placeholder="{{ __('message.video_url') }}" class="form-control">
                            </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-check">
                                            <div class="custom-control custom-radio d-inline-block col-4">
                                                ...
                                                <input type="radio" name="based" id="based-reps" value="reps" class="form-check-input" {{ old('based', 'reps') == 'reps' ? 'checked' : '' }}>
                                            </div>
                                            <div class="custom-control custom-radio d-inline-block col-4">
                                                <label class="form-check-label" for="based-time"> {{__('message.time')}}(s)</label>
                                                <input type="radio" name="based" id="based-time" value="time" class="form-check-input" {{ old('based') == 'time' ? 'checked' : '' }}>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <table id="table_list" class="table table-responsive">
                                            ...
                                            <tbody>
                                                ...
                                                <tr id="row_{{  $key }}" row="{{  $key }}" data-id="{{  $key }}">
                                                    <td>
                                                        <div class="form-group">
                                                            <input type="number" name="reps[]" value="{{ $field['reps'] ?? null }}" placeholder="{{ __('message.reps') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            <input type="number" name="time[]" value="{{ $field['time'] ?? null }}" placeholder="{{ __('message.time') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td class="weight">
                                                        <div class="form-group">
                                                            <input type="number" name="weight[]" value="{{ $field['weight'] ?? null }}" placeholder="{{ __('message.weight') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            <input type="number" name="rest[]" value="{{ $field['rest'] ?? null }}" placeholder="{{ __('message.rest') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td>
                                                ...
                                                <tr id="row_0" row="0" data-id="0">
                                                    <td>
                                                        <div class="form-group">
                                                            <input type="number" name="reps[]" value="{{ old('reps.0') }}" placeholder="{{ __('message.reps') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            <input type="number" name="time[]" value="{{ old('time.0') }}" placeholder="{{ __('message.time') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td class="weight">
                                                        <div class="form-group">
                                                            <input type="number" name="weight[]" value="{{ old('weight.0') }}" placeholder="{{ __('message.weight') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            <input type="number" name="rest[]" value="{{ old('rest.0') }}" placeholder="{{ __('message.rest') }}" class="form-control" min="0">
                                                        </div>
                                                    </td>
                            ...
                            <div id="exercise-duration" class="tab-pane fade {{ $type == 'duration' ? 'active show' : '' }}" role="tabpanel">
                                <div class="row duration_row">
                                    ...
                                    <div class="form-group col-md-2">
                                        <label for="hours" class="form-control-label">{{ __('message.hours') }} <span class="text-danger">*</span></label>
                                        <select name="hours" id="hours" class="form-control select2js" data-placeholder="{{ __('message.select_name',[ 'select' => __('message.hours') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'hours']) }}">
                                            @if(isset($duration))
                                                <option value="{{ $duration[0] }}" selected>{{ $duration[0] }}</option>
                                            @endif
                                        </select>
                                    </div>
                                    
                                    <div class="form-group col-md-2">
                                        <label for="minute" class="form-control-label">{{ __('message.minute') }} <span class="text-danger">*</span></label>
                                        <select name="minute" id="minute" class="form-control select2js" data-placeholder="{{ __('message.select_name',[ 'select' => __('message.minute') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'minute']) }}">
                                            @if(isset($duration))
                                                <option value="{{ $duration[1] }}" selected>{{ $duration[1] }}</option>
                                            @endif
                                        </select>
                                    </div>
                                    <div class="form-group col-md-2">
                                        <label for="second" class="form-control-label">{{ __('message.second') }} <span class="text-danger">*</span></label>
                                        <select name="second" id="second" class="form-control select2js" data-placeholder="{{ __('message.select_name',[ 'select' => __('message.second') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'second']) }}">
                                            @if(isset($duration))
                                                <option value="{{ $duration[2] }}" selected>{{ $duration[2] }}</option>
                                            @endif
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="form-group col-md-12">
                            <label for="instruction" class="form-control-label">{{ __('message.instruction') }}</label>
                            <textarea name="instruction" id="instruction" class="form-control tinymce-instruction" placeholder="{{ __('message.instruction') }}">{{ old('instruction', isset($data) ? $data->instruction : '') }}</textarea>
                        </div>
                        <div class="form-group col-md-12">
                            <label for="tips" class="form-control-label">{{ __('message.tips') }}</label>
                            <textarea name="tips" id="tips" class="form-control tinymce-tips" placeholder="{{ __('message.tips') }}">{{ old('tips', isset($data) ? $data->tips : '') }}</textarea>
                        </div>                        
                        <hr>

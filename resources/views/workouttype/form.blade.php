...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            <form action="{{ route('workouttype.update', $id) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
        @else
            <form action="{{ route('workouttype.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
        @endif
        <div class="row">
            <div class="col-lg-12">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="">
                                    <input class="form-control file-input" type="file" name="workouttype_image" accept="image/*">
                                </div>
                            </div>
                            
                            @if( isset($id) && getMediaFileExit($data, 'workouttype_image'))
                                <div class="col-md-2 mb-2 position-relative">
                                    <img id="workouttype_image_preview" src="{{ getSingleMedia($data,'workouttype_image') }}" alt="workouttype-image" class="avatar-100 mt-1">
                                    <a class="text-danger remove-file" href="{{ route('remove.file', ['id' => $data->id, 'type' => 'workouttype_image']) }}"

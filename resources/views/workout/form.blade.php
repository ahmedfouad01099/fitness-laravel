...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            <form action="{{ route('workout.update', $id) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
        @else
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                <label for="workout_type_id" class="form-control-label">{{ __('message.workouttype') }} <span class="text-danger">*</span></label>
                                <select name="workout_type_id" id="workout_type_id" class="select2js form-group workouttype" data-placeholder="{{ __('message.select_name',[ 'select' => __('message.workouttype') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'workout_type']) }}" required>
                                    @if(isset($id) && $data->workouttype)
                                        <option value="{{ $data->workouttype->id }}" {{ old('workout_type_id') == $data->workouttype->id ? 'selected' : '' }}>{{ $data->workouttype->title }}</option>
                                    @endif
                                </select>
                            </div>
                                            ...
                                            <tr id="row_{{ $key }}" row="{{ $key }}" data-id="{{ $field->id }}">
                                                ...
                                                <td>
                                                    <div class="form-group" id="exercise_ids_{{$key}}">
                                                        ...
                                                        <select name="exercise_ids[{{$key}}][]" id="exercise_ids_{{$key}}" class="select2tagsjs form-group exercise" multiple data-placeholder="{{ __('message.select_name',[ 'select' => __('message.exercise') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'exercise']) }}">
                                                            @if(isset($field->exercise_data) && count($field->exercise_data) > 0)
                                                                @foreach($field->exercise_data as $exerciseKey => $exerciseValue)
                                                                    <option value="{{ $exerciseKey }}" {{ in_array($exerciseKey, $field->exercise_ids ?? []) ? 'selected' : '' }}>{{ $exerciseValue }}</option>
                                                                @endforeach
                                                            @endif
                                                        </select>
                                                    </div>
                                                ...
                                                <td>
                                                    <div class="form-group">
                                                        ...
                                                        <input type="checkbox" name="is_rest[{{$key}}]" id="is_rest_yes_{{$key}}" value="1" class="form-check-input" {{ ($field->is_rest ?? null) ? 'checked' : '' }}>
                                                    </div>
                                                </td>
                                                <td>
                                        ...
                                        <tr id="row_0" row="0" data-id="0">
                                            ...
                                            <td>
                                                <div class="form-group" id="exercise_ids_0">
                                                    ...
                                                    <select name="exercise_ids[0][]" id="exercise_ids_0" class="select2tagsjs form-group exercise" multiple data-placeholder="{{ __('message.select_name',[ 'select' => __('message.exercise') ]) }}" data-ajax--url="{{ route('ajax-list', ['type' => 'exercise']) }}">
                                                        @if(old('exercise_ids.0'))
                                                            @foreach(old('exercise_ids.0') as $exerciseId)
                                                                <option value="{{ $exerciseId }}" selected>{{ $exerciseId }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                            ...
                                            <td>
                                                <div class="form-group">
                                                    ...
                                                    <input type="checkbox" name="is_rest[0]" id="is_rest_yes_0" value="1" class="form-check-input" {{ old('is_rest.0') ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            <td>

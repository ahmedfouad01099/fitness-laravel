...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            <form action="{{ route('level.update', $id) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
        @else
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="card-action">
                            <a href="{{ route('level.index') }} " class="btn btn-sm btn-primary" role="button">{{ __('message.back') }}</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                <label for="rate" class="form-control-label">{{ __('message.rate') }} <span class="text-danger">*</span></label>
                                <input type="number" name="rate" id="rate" value="{{ old('rate', isset($data) ? $data->rate : '') }}" placeholder="{{ __('message.rate') }}" class="form-control" required>
                            </div>
                            <div class="form-group col-md-6">
                            ...
                            @if( isset($id) && getMediaFileExit($data, 'level_image'))
                                <div class="col-md-2 mb-2 position-relative">
                                    <img id="category_image_preview" src="{{ getSingleMedia($data,'level_image') }}" alt="level-image" class="avatar-100 mt-1">
                                    <a class="text-danger remove-file" href="{{ route('remove.file', ['id' => $data->id, 'type' => 'level_image']) }}"

<form action="{{ route('settingUpdate') }}" method="POST" data-toggle="validator">
    @csrf
    <input type="hidden" name="id" value="{{ $setting_value->first()->id ?? '' }}" class="form-control"> <input
        type="hidden" name="page" value="{{ $page }}" class="form-control">
    <div class="row">
        ...
        <div class="col-md-12 col-sm-12 card shadow mb-10">
            ...
            <div class="card-body">
                <div class="row">
                    @foreach($value as $sub_keys => $sub_value)
                        ...
                        <div class=" {{ $class }} col-sm-12">
                            <div class="form-group">
                                <label for="{{ $key . '_' . $sub_keys }}">{{ str_replace('_', ' ', $sub_keys) }}
                                    {!! $sub_keys == 'TIME' ? '<span data-toggle="tooltip" title="' . __('message.set_quote_time', ['timezone' => config('app.timezone')]) . '">&#9432;</span>' : '' !!}</label>
                                ...
                                @elseif($key == 'CURRENCY' && $sub_keys == 'POSITION')
                                    <select name="value[]" class="form-control select2js">
                                        <option value="left" {{ (isset($data) && $data->value == 'left') ? 'selected' : '' }}>
                                            {{ __('message.left') }}</option>
                                        <option value="right" {{ (isset($data) && $data->value == 'right') ? 'selected' : '' }}>
                                            {{ __('message.right') }}</option>
                                    </select>
                                @elseif($key == 'QUOTE' && $sub_keys == 'TIME')
                                    <input type="{{ $type }}" name="value[]" value="{{ isset($data) ? $data->value : null }}"
                                        id="{{ $key . '_' . $sub_keys }}" class="form-control form-control-lg timepicker24"
                                        placeholder="{{ str_replace('_', ' ', $sub_keys) }}">
                                @else
                                    <input type="{{ $type }}" name="value[]" value="{{ isset($data) ? $data->value : null }}"
                                        id="{{ $key . '_' . $sub_keys }}" {{ $type == 'number' ? "min=0 step='any'" : '' }}
                                        class="form-control form-control-lg" placeholder="{{ str_replace('_', ' ', $sub_keys) }}">
                                @endif
                            </div>
                        </div>
                    @endforeach
                    <div class="col-md-12">
                        <button type="submit"
                            class="btn btn-md btn-primary float-md-end">{{ __('message.save') }}</button>
                    </div>
                </div>
            </div>
        </div>

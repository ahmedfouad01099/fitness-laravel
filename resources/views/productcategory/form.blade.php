...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            <form action="{{ route('productcategory.update', $id) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
        @else
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="card-action">
                            <a href="{{ route('productcategory.index') }}" class="btn btn-sm btn-primary" role="button">{{ __('message.back') }}</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="">
                                    <input class="form-control file-input" type="file" name="productcategory_image" accept="image/*">
                                </div>
                            </div>
                            
                            @if( isset($id) && getMediaFileExit($data, 'productcategory_image'))
                                <div class="col-md-2 mb-2 position-relative">
                                    <img id="productcategory_image_preview" src="{{ getSingleMedia($data,'productcategory_image') }}" alt="productcategory-image" class="avatar-100 mt-1">
                                    <a class="text-danger remove-file" href="{{ route('remove.file', ['id' => $data->id, 'type' => 'productcategory_image']) }}"

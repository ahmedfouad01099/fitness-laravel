...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            <form action="{{ route('equipment.update', $id) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
        @else
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="">
                                    <input class="form-control file-input" type="file" name="equipment_image" accept="image/*">
                                </div>
                            </div>
                            
                            @if( isset($id) && getMediaFileExit($data, 'equipment_image'))
                                <div class="col-md-2 mb-2 position-relative">
                                    <img id="equipment_image_preview" src="{{ getSingleMedia($data,'equipment_image') }}" alt="equipment-image" class="avatar-100 mt-1">
                                    <a class="text-danger remove-file" href="{{ route('remove.file', ['id' => $data->id, 'type' => 'equipment_image']) }}"
